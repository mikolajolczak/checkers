...
public class Move {
  ...
  public boolean checkAllPiecesPossibleTakes(int color, int colorQueen, BoardState bs) {
    for (int row = 0; row < GameConstants.BOARD_SIZE; row++) {
      for (int col = 0; col < GameConstants.BOARD_SIZE; col++) {
        ...
        if ((piece == color || piece == colorQueen) && canITake(col, row, bs)) return true;
      }
    }
    return false;
  }
  ...
  public boolean canKingTake(int row, int col, int piece, BoardState bs) {
    int enemy1 = (piece == GameConstants.RED_KING) ? GameConstants.BLACK : GameConstants.RED;
    int enemy2 = (piece == GameConstants.RED_KING) ? GameConstants.BLACK_KING : GameConstants.RED_KING;
    ...
    for (int[] dir : GameConstants.DIRECTIONS) {
      for (int dist = 1; dist < GameConstants.BOARD_SIZE - 1; dist++) {
        ...
        if (check == enemy1 || check == enemy2) {
          ...
          if (isValidPosition(landR, landC) && bs.getPiece(landR, landC) == GameConstants.EMPTY) return true;
          break;
        } else if (check != GameConstants.EMPTY) break;
      }
    }
    return false;
  }
  ...
  public boolean isItLegalSecondClickMove(int c2, int r2, int c1, int r1, int color) {
    ...
    switch (color) {
      case GameConstants.RED_KING:
      case GameConstants.BLACK_KING:
        if (isItOnTheSameDiagonal(c1, r1, c2, r2)) return false;
        int dc = Integer.compare(c2, c1), dr = Integer.compare(r2, r1);
        for (int i = r1 + dr, j = c1 + dc; i != r2 || j != c2; i += dr, j += dc) {
          if (boardState.getPiece(i, j) != GameConstants.EMPTY) return false;
        }
        return true;
      case GameConstants.RED:
